...
import { DAY_END_HOUR, DAY_HOUR_HEIGHT, DAY_START_HOUR, formatHour24 } from 'app/utils/day';
...
export const DayCalendar = forwardRef<IDayCalendarHandle, IDayCalendarProps>(function DayCalendar(
  { events, onDateChange },
  ref,
) {
        ...
        <div className='absolute inset-0'>
          {resolvedEvents?.map((event) => (
            <DayEventCard
              key={event.id}
              event={event}
              offsetTop={timeToOffset(event.startTime, DAY_START_HOUR, DAY_HOUR_HEIGHT)}
              height={durationToHeight(event.startTime, event.endTime, DAY_HOUR_HEIGHT)}
            />
          ))}
