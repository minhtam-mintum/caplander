...
import { Bold, CalendarDays, Clock, Italic, Link2, List, Pencil, Plus, Tag, X } from 'lucide-react';
...
function makeDefaultForm(data?: Partial<EventFormData>): EventFormData {
...
export const EventModal = forwardRef<IEventModalHandle, IEventModalProps>(
  function EventModal({ isOpen: controlledIsOpen, onClose: controlledOnClose, initialData }, ref) {
    const [internalIsOpen, setInternalIsOpen] = useState(false);
    const [internalData, setInternalData] = useState<Partial<EventFormData> | undefined>(undefined);
    const [form, setForm] = useState<EventFormData>(() => makeDefaultForm(initialData));
    const [editingTitle, setEditingTitle] = useState(false);

    const resolvedIsOpen = controlledIsOpen ?? internalIsOpen;
    const resolvedData = internalData ?? initialData;
    ...
    // Reset form whenever the modal opens imperatively with new data
    useEffect(() => {
      if (internalIsOpen) setForm(makeDefaultForm(resolvedData));
    }, [internalIsOpen]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    return (
      <Modal isOpen={resolvedIsOpen} onClose={handleClose}>
        {/* Header */}
